...
const CharacterTraits = ({traits, edit, addNewTrait, removeTrait}: CharacterTraitsProps )=> {
            ...
            <Accordion.Item eventKey={trait.index} key={trait.index}>
                ...
                <Accordion.Body>
                    <Row>
                        ...
                        <Col md="auto" className="d-flex justify-content-end mb-auto mt-1">
                            {edit && removeTrait && <Button variant="outline-danger" size="sm" style={{marginRight:"10px"}} onClick={() => {removeTrait(trait.index)}}>
    ...
    const onAddClick = () => {
        ...
        if(trimmedName && (traits?.filter((trait) => trait.index === index).length === 0)){
            setIsValidName(true);
            console.log('add')
    ...
    const addTrait = () => {
        return (
            <Accordion.Item eventKey="Add-Trait">
                <Accordion.Header><PlusCircle style={{marginRight: "10px"}}/>Add a trait</Accordion.Header>
                <Accordion.Body>
                    <Row>
                        <FormGroup>
                            <Form.Label>Name</Form.Label>
                            <Form.Control type="text" onChange={onTraitNameChange} value={traitName}/>
                            {!isValidName && <Form.Text>Please enter a unique name for this trait</Form.Text>}
                        </FormGroup>
                    </Row>
                    <Row>
                        <FormGroup>
                            <Form.Label>Info</Form.Label>
                            <Form.Control type="text" as="textarea" onChange={onTraitInfoChange} value={traitInfo}/>
                        </FormGroup>
                    </Row>
