...
import { Equipment, EquipmentCategory, EquipmentData } from "../../../constants/types"
...
describe("EquipmentTab", () => {
    beforeEach(() => {
        ...
        (global.fetch as jest.Mock).mockResolvedValue(Promise.resolve(
            {
                ok: true,
                json: jest.fn().mockResolvedValue(Promise.resolve({ equipment: mockAllEquipment }))
            }
        ))
    });
    ...
    test("renders component", async () => {
        render(<EquipmentTab allEquipment={mockAllEquipment} categories={mockCategories} />);
    ...
    test("submit button click", async () => {
        ...
        expect(await screen.findByTestId("item-card-undefined")).toBeInTheDocument();
