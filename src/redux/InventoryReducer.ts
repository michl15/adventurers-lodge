...
export const inventorySlice = createSlice({
    ...
    reducers: {
        ...
        updateItemQuantity: (state, action: PayloadAction<{ value: number, index: number }>) => {
            if (action.payload.value === 0) {
                const newList = state.inventoryList.splice(action.payload.index, 1);
                state.inventoryList = [...state.inventoryList]
            } else {
                state.inventoryList[action.payload.index].quantity = action.payload.value;
            }
        },
...
export const { addItem, updateItemQuantity, resetInventory, setInventory } = inventorySlice.actions;
